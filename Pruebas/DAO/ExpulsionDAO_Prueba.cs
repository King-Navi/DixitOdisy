...
namespace Pruebas.DAO
{
    ...
    public class ExpulsionDAO_Prueba : ConfiguracionPruebaBD
    {
        ...
        public void LimpiarRegistrosDePrueba()
        {
            ...
            {
                var expulsiones = context.Expulsion.Where(expulsion => expulsion.idUsuarioCuenta == IDUSUARIOCUENTA_PRUEBA).ToList();
        ...
        {
            bool resultado = expulsionDAO.CrearRegistroExpulsion(ID_INVALIDO, MOTIVO_PRUEBA, ES_HACKER_FALSE);
        ...
        public void CambiarExpulsionesAFueronPenalizadas_CuandoHayExpulsionesSinPenalizar_DeberiaActualizarEstado()
        {
            ...
            using (var context = new DescribeloEntities())
            {
                var expulsiones = context.Expulsion
                                          .Where(expulsion => expulsion.idUsuarioCuenta == IDUSUARIOCUENTA_PRUEBA)
                                          .ToList();

                Assert.IsTrue(expulsiones.All(expulsion => expulsion.fuePenalizado), "Todas las expulsiones deberían estar penalizadas.");
            }
        }
        [TestMethod]
        ...
        {
            bool resultado = expulsionDAO.CambiarExpulsionesAFueronPenalizadas(ID_INVALIDO);
